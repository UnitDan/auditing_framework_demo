...
class DataGenerator():
    ...
    def generate_around_datapoint(self, datapoint, range_ratio=0.1, n=1, with_sensitive_feature=None):
        normed_datapoint = self.norm(datapoint).squeeze()
        normed_random_noise = torch.rand((n, normed_datapoint.shape[0]))*range_ratio
        
        normed_data_around = normed_datapoint + normed_random_noise
        data_around = self.recover(normed_data_around)
        data_around = self.clip(data_around, with_sensitive_feature=with_sensitive_feature)
